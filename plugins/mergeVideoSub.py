...
async def mergeSub(c: Client, cb: CallbackQuery, new_file_name: str):
    ...
    if subbed_video is None:
        await cb.message.edit("**Failed to add subtitle to video !**")
        await delete_all(root=f"downloads/{str(cb.from_user.id)}")
        queueDB.update({cb.from_user.id: {"videos": [], "subtitles": [], "audios": []}})
        formatDB.update({cb.from_user.id: None})
        return
    try:
        await cb.message.edit("**Adding subtitle to video completed !**")
    except MessageNotModified:
        await cb.message.edit("**Adding subtitle to video is completed !**")
    ...
    if Config.IS_PREMIUM and file_size > 4241280205:
        ...
        formatDB.update({cb.from_user.id: None})
        return
    await cb.message.edit("**Uploading is about to start...**")
    ...
    try:
        ...
        formatDB.update({cb.from_user.id: None})
        await cb.message.edit(
            "<b>Merged Video is corrupted \n\n<i>Try setting custom thumbnail</i></b>",
        )
        return
    await uploadVideo(
        c=c,
